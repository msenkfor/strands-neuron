...
resource "aws_launch_template" "trn2" {
  ...
  # Boot validation — checks for exactly 16 EFA devices at startup.
  # If fewer than 16 are found (bad host/hardware fault), the instance
  # tags itself as EFA_CHECK=FAILED and shuts itself down automatically
  # so you don't waste time SSHing into a broken instance.
  # Results are logged to /var/log/efa-check.log on the instance.
  user_data = base64encode(<<-EOF
    #!/bin/bash
    set -e
    LOGFILE=/var/log/efa-check.log
    EXPECTED=16
    INSTANCE_ID=$(curl -s http://169.254.169.254/latest/meta-data/instance-id)
    REGION=$(curl -s http://169.254.169.254/latest/meta-data/placement/region)

    echo "=== EFA Device Check ===" | tee $LOGFILE
    echo "Timestamp: $(date -u)" | tee -a $LOGFILE
    echo "Instance: $INSTANCE_ID" | tee -a $LOGFILE

    # Wait for PCI devices to fully enumerate
    sleep 10

    EFA_COUNT=$(lspci | grep -c "Elastic Fabric Adapter" || true)
    echo "EFA devices found: $EFA_COUNT / $EXPECTED" | tee -a $LOGFILE

    # List all EFA BDFs for reference
    echo "EFA BDFs:" | tee -a $LOGFILE
    lspci | grep "Elastic Fabric" | tee -a $LOGFILE

    if [ "$EFA_COUNT" -lt "$EXPECTED" ]; then
      echo "FAILED: Only $EFA_COUNT EFA devices found, expected $EXPECTED." | tee -a $LOGFILE
      echo "This host has a hardware fault. Tagging instance and shutting down." | tee -a $LOGFILE

      # Tag the instance so you can see the failure reason in the EC2 console
      aws ec2 create-tags \
        --region $REGION \
        --resources $INSTANCE_ID \
        --tags Key=EFA_CHECK,Value=FAILED Key=EFA_FOUND,Value=$EFA_COUNT

      # Shut down so Terraform can detect the instance is gone and re-launch
      shutdown -h now
    else
      echo "PASSED: All $EFA_COUNT EFA devices present." | tee -a $LOGFILE

      aws ec2 create-tags \
        --region $REGION \
        --resources $INSTANCE_ID \
        --tags Key=EFA_CHECK,Value=PASSED Key=EFA_FOUND,Value=$EFA_COUNT
    fi
  EOF
  )
